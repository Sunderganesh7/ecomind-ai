...
import { 
  ArrowLeft, RefreshCw, Map as MapIcon, ShieldCheck, ChevronDown, ChevronUp, Link as LinkIcon, BookOpen, Tractor, Layers
} from 'lucide-react';
...
export const LandIntelligence: React.FC = () => {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
          <div style={{ maxWidth: '650px' }}>
            ...
            <p style={{ color: 'var(--color-text-secondary)', marginTop: '8px', fontSize: '1rem', lineHeight: 1.5 }}>
              Understand land use efficiency, agronomic impact, and landscape integrity.
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            ...
            <div style={{ background: 'var(--color-warning-bg)', borderRadius: '12px', padding: '16px', display: 'flex', gap: '16px', alignItems: 'flex-start', width: '240px', position: 'relative', overflow: 'hidden', border: '1px solid rgba(245, 158, 11, 0.2)' }}>
              <div style={{ background: 'var(--color-warning)', color: 'white', borderRadius: '50%', padding: '8px', display: 'flex', zIndex: 1 }}>
                <MapIcon size={18} />
              </div>
              <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--color-primary-dark)', zIndex: 1, lineHeight: 1.4 }}>
                Optimal Usage<br/>Sustainable Practices<br/>Higher Yields
              </div>
            </div>
          </div>
        </div>
  ...
  const landUse = safeVal(profile.land?.use || profile.land?.land_use);
  const crop = safeVal(profile.land?.crop);
  const croppingSystem = safeVal(profile.land?.cropping_system);
  
  const landAnalysis = getMetricAnalysis('land_use');
  const cropAnalysis = getMetricAnalysis('crop');
  const sysAnalysis = getMetricAnalysis('cropping_system');
  ...
  let landStatus = 'Unknown';
  if (rawStatus && rawStatus !== 'Unknown') {
    landStatus = String(rawStatus);
  } else if (safeVal(profile.land_use?.land_use_type) !== 'Unknown' || safeVal(profile.land_use?.primary_crop) !== 'Unknown' || safeVal(profile.land_use?.cropping_system) !== 'Unknown') {
    landStatus = 'Context Dependent';
  }
  ...
  const pageRels = (relationships?.relationships || []).filter(r => {
    ...
    return sources.some(v => v.toLowerCase().includes('land') || v.toLowerCase().includes('crop') || v.toLowerCase().includes('habitat')) ||
      target.toLowerCase().includes('land') || target.toLowerCase().includes('crop') || target.toLowerCase().includes('habitat');
  });

  const pageRisks = (risks?.risk_patterns || []).filter(r => {
    const vars = r.variables_used || [];
    return vars.some(v => v.toLowerCase().includes('land') || v.toLowerCase().includes('crop') || v.toLowerCase().includes('habitat')) ||
      r.name.toLowerCase().includes('land') || r.name.toLowerCase().includes('habitat');
  });
    ...
    <div className="main-content" style={{ width: '92%', margin: '0 auto', maxWidth: '1400px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
        <div style={{ maxWidth: '650px' }}>
          ...
          <p style={{ color: 'var(--color-text-secondary)', marginTop: '8px', fontSize: '1rem', lineHeight: 1.5 }}>
            Understand land use, cropping systems, habitat structure, and their environmental implications.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
          ...
          <div style={{ background: 'var(--color-warning-bg)', borderRadius: '12px', padding: '16px', display: 'flex', gap: '16px', alignItems: 'flex-start', width: '240px', position: 'relative', overflow: 'hidden', border: '1px solid rgba(245, 158, 11, 0.2)' }}>
            ...
            <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--color-primary-dark)', zIndex: 1, lineHeight: 1.4 }}>
              Sustainable Land<br/>Resilient Yields<br/>Healthy Habitats
            </div>
            <svg style={{ position: 'absolute', right: '-15px', top: '5px', opacity: 0.08, zIndex: 0, transform: 'rotate(15deg)' }} width="80" height="80" viewBox="0 0 24 24" fill="var(--color-warning)">
              <path d="M15 2L5 6V20L15 16L25 20V6L15 2Z" />
            </svg>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <div style={{ ...cardStyle, position: 'relative' }}>
            ...
            <h2 style={{ fontSize: '1.25rem', marginBottom: '24px', color: 'var(--color-primary-dark)' }}>Current Land Condition</h2>
            ...
            <div style={{ display: 'flex', gap: '32px', alignItems: 'stretch' }}>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: 'var(--color-surface-hover)', borderRadius: '12px', padding: '24px' }}>
                ...
                <span style={{ fontSize: '2.5rem', fontWeight: 800, color: landStatus.toLowerCase().includes('optimal') ? 'var(--color-success)' : (landStatus.toLowerCase().includes('critical') ? 'var(--color-danger)' : 'var(--color-warning)'), textTransform: 'uppercase', lineHeight: 1, textAlign: 'center' }}>
                  {landStatus.replace(/_/g, ' ')}
                </span>
              </div>
              ...
              <div style={{ flex: 2, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ color: 'var(--color-text-secondary)', display: 'flex', alignItems: 'center', gap: '8px' }}><MapIcon size={16} color="var(--color-warning)" /> Land Use</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <span style={{ fontWeight: 600 }}>{landUse.replace(/_/g, ' ')}</span>
                      <span style={{ ...badgeStyle(landAnalysis.status), padding: '4px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 600 }}>{landAnalysis.status.replace(/_/g, ' ').toUpperCase()}</span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ color: 'var(--color-text-secondary)', display: 'flex', alignItems: 'center', gap: '8px' }}><Tractor size={16} color="var(--color-success)" /> Primary Crop</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <span style={{ fontWeight: 600 }}>{crop.replace(/_/g, ' ')}</span>
                      <span style={{ ...badgeStyle(cropAnalysis.status), padding: '4px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 600 }}>{cropAnalysis.status.replace(/_/g, ' ').toUpperCase()}</span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ color: 'var(--color-text-secondary)', display: 'flex', alignItems: 'center', gap: '8px' }}><Layers size={16} color="var(--color-info)" /> Cropping System</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <span style={{ fontWeight: 600 }}>{croppingSystem.replace(/_/g, ' ')}</span>
                      <span style={{ ...badgeStyle(sysAnalysis.status), padding: '4px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 600 }}>{sysAnalysis.status.replace(/_/g, ' ').toUpperCase()}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div>
            <h2 style={{ fontSize: '1.25rem', marginBottom: '16px', color: 'var(--color-primary-dark)' }}>Land & Environmental Relationships</h2>
            {pageRels.length === 0 ? (
          ...
          <div>
            <h2 style={{ fontSize: '1.25rem', marginBottom: '16px', color: 'var(--color-primary-dark)' }}>Land Risks</h2>
            {pageRisks.length === 0 ? (
              <div style={cardStyle}><p style={{ color: 'var(--color-text-tertiary)', margin: 0 }}>No active risk patterns detected for current land data.</p></div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {pageRisks.map(r => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          <div style={{ ...cardStyle, background: 'var(--color-surface-hover)' }}>
            <h3 style={{ fontSize: '1.1rem', color: 'var(--color-primary-dark)', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <BookOpen size={20} /> LAND INTELLIGENCE
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <span style={{ display: 'block', fontSize: '0.85rem', color: 'var(--color-text-tertiary)', marginBottom: '4px' }}>Current Condition</span>
                <span style={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>{landStatus.replace(/_/g, ' ').toUpperCase()}</span>
              </div>
              <div>
                <span style={{ display: 'block', fontSize: '0.85rem', color: 'var(--color-text-tertiary)', marginBottom: '4px' }}>Key Indicators</span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  <span style={{ background: 'white', padding: '4px 10px', borderRadius: '12px', fontSize: '0.8rem', border: '1px solid var(--color-border)' }}>{landUse.replace(/_/g, ' ')}</span>
                  <span style={{ background: 'white', padding: '4px 10px', borderRadius: '12px', fontSize: '0.8rem', border: '1px solid var(--color-border)' }}>{crop.replace(/_/g, ' ')}</span>
                  <span style={{ background: 'white', padding: '4px 10px', borderRadius: '12px', fontSize: '0.8rem', border: '1px solid var(--color-border)' }}>{croppingSystem.replace(/_/g, ' ')}</span>
                </div>
              </div>
            </div>
          </div>
